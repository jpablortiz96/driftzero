...
**Decision**: schema validation is necessary but NOT sufficient. The defensive chain has five layers (plan.md, Tool Response Validation Chain): schema -> provenance / expected source -> expected artifact and tool identity -> authorization and scope -> deterministic semantic invariants in the Truth Engine. Layers 2-5 are Truth Engine responsibilities and are deterministic; no LLM judgment participates in accepting a tool response.
